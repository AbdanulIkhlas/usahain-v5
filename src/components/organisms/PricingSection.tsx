        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch pt-4">
          {plans.map((plan, idx) => (
            <motion.div
              ...
              className={`p-8 sm:p-10 rounded-3xl flex flex-col h-full relative transition-all duration-300 ${
                plan.highlight
                  ...
                  : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 shadow-lg hover:shadow-xl"
              }`}
            ...
            >
              ...
              <p
                className={`min-h-[3rem] mb-6 ${plan.highlight ? "text-white/80" : "text-slate-500 dark:text-slate-400"}`}
              >
                {plan.description}
              </p>
              ...
              <div className="mb-8 flex items-baseline gap-1">
                ...
                {plan.period && (
                  <span
                    className={`font-medium ${plan.highlight ? "text-white/70" : "text-slate-500 dark:text-slate-400"}`}
                  >
                    {plan.period}
                  </span>
                )}
              ...
              <ul className="flex flex-col gap-4 mb-10 flex-1">
                {plan.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-start gap-3">
                    ...
                    />
                    <span
                      className={`font-medium leading-relaxed ${plan.highlight ? "text-white/90" : "text-slate-700 dark:text-slate-200"}`}
                    >
                      {feature}
                    </span>
                  </li>
